...
globalThis.fetch=async(url,opts={})=>{
 ...
 const payload=await responder(request);return new Response(JSON.stringify(payload),{status:payload?.error?.name==='invalid_access_key'?401:(request.method==='POST'?201:200)});
};
...
const diagnose=await cli(['system','diagnose','--json']);const diagnosis=JSON.parse(diagnose.stdout);assert(diagnosis.checks.some(c=>c.name==='Auth Validity'&&c.status==='fail'));assert.equal(diagnose.exitCode,0);results.push({test:'diagnose_failed_auth_exit_zero',exitCode:diagnose.exitCode,checks:diagnosis.checks});
