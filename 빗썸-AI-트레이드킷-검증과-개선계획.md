...
> 범위 정정: 이 문서는 주로 배포 코드 검사 기록이다. 사용자가 요청한 공식 MD 지침 자체의 오류 검토는 [별도 주 문서](./빗썸-공식-MD-지침-오류검토.md)를 따른다. `tru`는 검사자가 만든 입력이며 공식 MD의 오타가 아니다. F01~F11을 MD 오류 목록으로 해석하지 않는다.
